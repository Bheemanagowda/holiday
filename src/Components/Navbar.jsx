...
function Navbar() {
  ...
  const [activeMobileRegion, setActiveMobileRegion] = useState(null);
                          ...
                          <div className="w-1/4 max-w-[220px] bg-gray-50 border-r border-gray-200 overflow-y-auto">
                            {ele.data.regions.map((region, rIdx) => (
                              <Link
                                ...
                                className={`block px-6 py-4 text-[15px] font-bold uppercase tracking-widest transition-all ${
  ...
  {ele.data.regions[desktopRegionIndex]?.sections.map((section) => (
    <div key={section.heading}>
      {section.link ? (
        <Link
          href={section.link}
          className="text-[#028680] font-black border-b border-gray-50 pb-2 mb-3 text-[14px] uppercase tracking-tighter block"
        >
          {section.heading}
        </Link>
      ) : (
        <h4 className="text-[#028680] font-black border-b border-gray-50 pb-2 mb-3 text-[14px] uppercase tracking-tighter">
          {section.heading}
        </h4>
      )}
      ...
      <ul className="space-y-1.5">
        {section.links.map((link) => (
          <li key={link.name}>
            <Link
              href={link.link}
              className="text-gray-600 hover:text-[#028680] transition-colors block text-[13px] font-normal"
            >
              {link.name}
            </Link>
          </li>
              ...
              <div key={uuidv4()} className="border-b border-gray-100">
                {!ele.type ? (
                  ...
                  <div>
                    ...
                    >
                      <span className="text-[15px] font-bold uppercase">{ele.name}</span>
                      ...
                      <div className="bg-gray-50">
                        {ele.data.regions.map((region, rIdx) => (
                          <Link key={region.name} href={region.link} onClick={() => setIsOpen(false)}>
                            <div className="border-b border-gray-200 last:border-0 px-6 py-4 flex justify-between items-center text-[13px] font-bold uppercase text-gray-800">
                              {region.name}
